...
import { CalendarDays, MousePointerClick, Send, TrendingUp } from "lucide-react";
...
export default function ClickRateChart({ data = [] }: Props) {
  const hasData =
    data.length > 0 &&
  ...
  const totalEmails = data.reduce((total, item) => total + item.value, 0);
  const averageEmails = data.length > 0 ? Math.round(totalEmails / data.length) : 0;
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid grid-cols-2 border-y border-slate-100 bg-slate-50/70 sm:grid-cols-3">
        ...
        <div className="col-span-2 hidden px-6 py-4 sm:col-span-1 sm:block">
          ...
          <p className="mt-1 text-xl font-bold tracking-tight text-slate-800">
            {hasData ? maxValue.toLocaleString() : "—"}
          </p>
        </div>
      </div>
            ...
            {data.map((item, idx) => (
              ...
              >
                <div className="relative flex w-full flex-1 items-end rounded-lg bg-slate-100/80 px-1.5 pt-2">
                  ...
                  <div
                    className="min-h-1.5 w-full rounded-md rounded-b-sm bg-gradient-to-t from-blue-600 to-blue-400 shadow-[0_-4px_12px_rgba(37,99,235,0.18)] transition-all duration-300 ease-out group-hover:from-blue-500 group-hover:to-sky-300"
                    style={{
        ...
        <div className="flex h-64 items-center justify-center px-6">
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-xs">
              Send campaigns to see daily email delivery trends here.
            </p>
          </div>
        </div>
      )}
